...
public class DataParser {
	...
	public String createAsciiName(final int startIndex, final int endIndex,
		...
		try {
			...
			DXTPerformanceTestingTool.printLine("Can't parse " + hexName
					+ " : " + e.getMessage() + "\n");
		} catch (final IndexOutOfBoundsException e) {
			DXTPerformanceTestingTool
					.printLine("Can't get value for interval ( "
	...
	public String createFormattedDate(final int startIndex, final int endIndex,
		...
		for (int i = 0; i <= length; i++) {
			...
			try {
				...
				DXTPerformanceTestingTool.printLine("Not defined value : "
						+ e.getMessage() + "\n");
			}
	...
	public Integer createIntValue(final int startIndex,
		...
		try {
			...
			DXTPerformanceTestingTool.printLine("Can't parse " + hexIntNumber
					+ " setting it to empty value" + "\n");
		}
	...
	public Long createLongValue(final int startIndex,
		...
		try {
			...
			DXTPerformanceTestingTool.printLine("Can't parse " + hexLongNumber
					+ " setting it to empty value" + "\n");
		} catch (final IndexOutOfBoundsException exc) {
			DXTPerformanceTestingTool
					.printLine("Can't get value for interval ( "
							+ (startIndex + difference) + "," + parsingIndex
							+ " ) .Setting it to empty value" + "\n");
		}
	...
	public Integer getFormatVersion(final List<String> hexValues) {
		...
		try {
			...
			DXTPerformanceTestingTool.printLine("Can't parse format version "
					+ hexValues.get(FORMAT_VERSION_BYTE) + " : "
					+ e.getMessage() + "\n");
		}
	...
	public Date parseTimeStamp(final int startIndex,
		...
		try {
			...
			date = cal.getTime();
			DXTPerformanceTestingTool.printLine("Report date =  "
					+ date.toString() + "\n");

		} catch (final Exception e) {
			DXTPerformanceTestingTool.printLine("Can't parse the date "
					+ hexValues.subList(18, 25) + " error: " + e.getMessage()
					+ "\n");
		}
		return date;
	}
	...
	public void printReportLogInHex(final List<String> hexValues) {
		...
		for (int i = 0; i < (hexValues.size() / 16) + 1; i++) {
			...
			if (i * 16 + 16 < hexValues.size()) {
				DXTPerformanceTestingTool.printLine(" Values ( " + i + " ) = "
						+ hexValues.subList(i * 16, i * 16 + 16) + "\n");
			} else {
				DXTPerformanceTestingTool.printLine(" Values ( " + i + " ) = "
						+ hexValues.subList(i * 16, hexValues.size()) + "\n");
			}
		}
	}
}
